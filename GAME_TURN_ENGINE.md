...
Подключены четыре процессора в таком порядке:

1. `PROVINCE_SEASONS` (приоритет `600`) меняет `season` во всех непустых провинциях.
2. `PROVINCE_CLIMATE` (приоритет `700`) рассчитывает поле `climate`.
3. `PROVINCE_FERTILITY` (приоритет `800`) рассчитывает поле `fertility` от `0` до `1` с сезонной поправкой.
4. `PROVINCE_REPORTS` (приоритет `900`) записывает понятный текст с эмодзи в `Провинции информация`.
